...
// Permite `node src/index.js --web --control-bot` sem depender de inline env vars
// (mais robusto que `KEY=value node ...` em diferentes shells/plataformas)
...
async function startOptionalServices() {
  ...
  if (process.env.ENABLE_WEB === 'true') {
    try {
      ...
      _stopWebServer = createWebServer(port);
    } catch (err) {
