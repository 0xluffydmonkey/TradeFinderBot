...
function getKeypair() {
  if (!_keypair) {
    ...
    logger.info(`[DRIFT] Wallet: ${_keypair.publicKey.toBase58()}`);
  }
  return _keypair;
}
...
export async function openPosition(tradeParams) {
  ...
  logger.info(`[DRIFT] Abrindo ${direction} ${asset} | Entrada: $${entry} | TP: $${tp} | SL: $${sl} | ${leverage}x`);
  ...
  logger.info(`[DRIFT] Enviando market order (ISOLATED ${effectiveLeverage.toFixed(2)}x)...`);
  const marketTxSig = await driftClient.placePerpOrder(
    marketOrderParams,
    undefined,            // txParams
    0,                    // subAccountId
    isolatedDepositAmount // → modo ISOLATED
  );
  ...
  logger.info(`[DRIFT] 🎯 Posição completa!`, {
...
function simulateTrade(tradeParams) {
  const { signalId, asset, direction, entry, tp, sl, leverage, positionSizeUSD, notionalValueUSD } = tradeParams;
