...
export function startPositionTracker(bot, chatIds, options = {}) {
  ...
  state.on('signal:executed', ({ signal, position }) => {
    const pos = position ?? state.positions.find(p => p.asset === signal?.asset);
    if (!pos) return;

    // Evitar duplicatas (caso o evento dispare mais de uma vez)
    if (cards.has(pos.asset)) return;

    createCard(pos, signal?.signalId).catch(err =>
      logger.error(`[TRACKER] Erro em createCard(${pos.asset}): ${err.message}`)
    );
  });
